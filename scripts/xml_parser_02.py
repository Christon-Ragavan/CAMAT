    ...
    def get_tie_information(self):
        ...
        for i in range(len(step)):
            ...
            if n == "restrest":
                n = "rest"
            notes.append(n)
        df_tie_info = pd.DataFrame(np.array([duration, step, octave, tie]).T, columns=["Duration", 'Pitch', 'Octave', 'Tie Type'])
        return df_tie_info
...
if __name__ == "__main__":
    ...
    num_voices = xml_tools.get_num_voices()
    ...
    pd_data = pd.DataFrame(np.array([offset_list, duration_list, pitch_comb_list, measure_num_list, chord_info_list, tie_info['Tie Type']]).T,
