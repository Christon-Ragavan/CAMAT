...
class XMLToolBox:
    def __init__(self, file_path):
        ...
        self.dict_12_pc = {'C': 0, 'C#': 1, 'D': 2, 'D#': 3, 'E': 4,'F': 5, 'F#': 6, 'G': 7, 'G#': 8, 'A': 9, 'A#': 10, 'B': 11}
    ...
    def convert_pitch_midi(self, df):
        ...
        self.midi_list = [self._pitch_to_midi(octave_list[i] , pitch_list[i])for i in range(len(pitch_list))]
    ...
    def compute_measure_offset(self, df):
        ...
        self.measure_off_sparse, self.n_measure_change_idx, measure_offset_sum = xml_tools._measure_offset_sparse(measure_num_list,
                                                                                                        self.measure_offset_list,
                                                                                                        idx_new_measure_offsets)
    ...
    def compute_voice_offset(self, df):
        print(self.measure_offset_list)
        measure_num_list = list(np.squeeze(df['Measure'].to_numpy(dtype=int)))
        ...
        nn_voice_list = []
        nn_measure_list = []
        print("offset_ml_num_list",offset_ml_num_list)
        ...
        for i, v in enumerate(voices_list):
            ...
            if i in self.n_measure_change_idx:
                measure_C+=1
            ...
            if i == 0:
                c_off = 0.0
                print("First", voices_list[i], v)
            else:
                c_ml_off = offset_ml_num_list[i]
                if voices_list[i - 1] != v:
    ...
    def compute_chord_offset(self, df):
        ...
        print("n_num_voice, n_set_voice", n_num_voice, n_set_voice)
...
def _merge_measure_duration_and_get_offset(df, measure_offset):
    ...
    measure_off_sparse, n_measure_change_idx, measure_offset_sum = _measure_offset_sparse(measure_num_list,
                                                                                          measure_offset,
                                                                                          idx_new_measure_offsets)
...
def plotting_wrapper(df):
    ...
    from hfm.scripts_in_progress.xml_parser.scripts.utils import _create_pianoroll_single
...
if __name__ == "__main__":
    ...
    #df_data_c = xml_tools.compute_chord_offset(df_data_v)
