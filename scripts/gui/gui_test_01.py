...
def search_button(data):
    x1 = entry1.get()

    label3 = tk.Label(root, text='--> ' + data+ x1 + ' <--', font=('helvetica', 20))
    canvas1.create_window(200, 210, window=label3)

    label4 = tk.Label(root, text=x1, font=('helvetica', 20, 'bold'))
    canvas1.create_window(200, 1030, window=label4)
...
if __name__ == "__main__":
    ...
    label1 = tk.Label(root, text='Search for keyword')
    label1.config(font=('helvetica', 14))
    canvas1.create_window(200, 25, window=label1)

    label2 = tk.Label(root, text='Enter Keyword for search:')
    ...
    action_with_arg = partial(search_button, data)
