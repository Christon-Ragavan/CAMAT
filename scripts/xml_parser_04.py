    ...
    def get_measure_info(self):
        ...
        for i in range(len(step)):
            ...
            if n == "restrest":
                n = "rest"
            notes.append(n)
        df_tie_info = pd.DataFrame(np.array([self.note_counter_list, duration, step, octave, self.measure_num_per_note_list, self.voice_num, chord_tags, tie]).T,
                                   columns=["#Note","Duration", 'Pitch', 'Octave', 'Measure','Voice', 'Chord Tags', 'Tie Type'])
        print(df_tie_info)
...
if __name__ == "__main__":
    ...
    tie_info = xml_tools.get_measure_info()