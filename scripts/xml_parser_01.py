...
class XMLHandler(xml.sax.ContentHandler):
    ...
    def startElement(self, tag, attrib):
        ...
        if self.tag == "chord":
            print(f".. @@@@@ YES CHORD IS HERE {self.tag}")
    ...
    def endElement(self, textcontent):
        if self.tag == 'duration':
            ...
            print("..", self.tag, "division :", self.divisions, "duration :", self.duration, "Actual Duration:", self.curr_duration)
            duration_list.append(self.curr_duration)
        if self.tag == 'note':
        ...
        if self.tag == 'step':
            ...
            print(f".. {self.tag} {self.note_name} {self.curr_measure_duration} {self.curr_measure_num} {self.measure_offset}")
        if self.tag == 'octave':
            self.curr_oct = int(self.octave)
            octave_list.append(self.curr_oct)
        ...
        if self.tag == 'beat-type':
            ...
            measure_offset.append(self.curr_measure_duration)
        ...
        if self.note_info != '':
            if self.chord_available==False:
                chord_info_list.append("xx")
            else:
                self.chord_available=False
            pitch_list.append(self.note_info)
            measure_num_list.append(int(self.curr_measure_num))
...
def _get_file():
    # path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/test_case_xml_parser_example.xml"
    # path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/test_case_tied_note.xml"
    # path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/ultimate_tie_test.xml"
    # path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/ultimate_tie_test2.xml"
    ...
    path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/stupid2.xml"
...
def _merge_measure_duration_and_get_offset(measure_offset, measure_num_list, duration_list, chord_info_list):
    ...
    for i, c in enumerate(chord_info_list):
        ...
        print(i , c)
    print("Latest : ", nn_offset_list, len(nn_offset_list))
    return nn_offset_list
if __name__=="__main__":
    """
    
    score_path = "/Users/chris/DocumentLocal/workspace/hfm/weimar/example_files/ultimate_tie_test3.xml"
    quantization = 8
    ...
    print("####################################################")
    print(f"measure_offset {len(measure_offset)} pitch {len(pitch_list)} duration {len(duration_list)} Oct {len(octave_list)}")
    print(f"measure num {len(measure_num_list)} pitch_comb_list {len(pitch_comb_list)} chord_info_list {len(chord_info_list)}")
    print("####################################################")
    ...
    offset_list = _merge_measure_duration_and_get_offset(measure_offset=measure_offset, measure_num_list=measure_num_list, duration_list=duration_list, chord_info_list=chord_info_list)


    pd_data = pd.DataFrame(np.array([offset_list ,duration_list, pitch_comb_list, measure_num_list, chord_info_list]).T, columns=['Offset', 'Duration', 'Pitch', 'Measure', 'chord_info'])
    print(pd_data)
    """
    12/8 => 6  
    11/8 => 5.5
    ...
    3/6 => x/4 ----- 
    quantization = 16   
    pianoroll, articulation = scoreToPianoroll(path, quantization)
    print(pianoroll)
    print(articulation)
    """
