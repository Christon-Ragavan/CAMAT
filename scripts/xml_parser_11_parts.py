...
def _get_file(search_keywords, extract_extire_database):
...
if __name__ == "__main__":
    ...
    paths = _get_file(search_keywords,extract_extire_database=True)
    c= 0
    e=0
    ...
    b = "/Users/chris/DocumentLocal/workspace/hfm/scripts_in_progress/xml_parser/xml_files/"
    print(correct_list)
    np.savetxt(b+'correct_parsed.csv', correct_list,delimiter =", ", fmt ='% s')
    np.savetxt(b+'error_parsed.csv', error_list, delimiter =", ", fmt ='% s')
    print("TOTAL Files ERROR", len(error_list))
    for i in error_list:
        print(i)
    print("TOTAL Files ERROR", len(error_list))
    print("TOTAL Files correct", len(correct_list))
